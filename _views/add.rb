  ...
  class Add < Jekyll::Component
    ...
    def view_template
      ...
      div(class: "container group", data: controller) do
        div class: "message hidden group-data-[add-state-value=initial]:flex", data: stimulus_item( actions: { "click" => :search }, for: Controllers::ADD) do
          Spinner()
    ...
    def subscribe_form
      form data: stimulus_item(target: :subscribe_form, actions: { "submit" => :"subscribe:prevent" }, for: Controllers::ADD), action: build_url("subscribe"), class: "hidden container group-data-[add-state-value=hasResults]:flex", novalidate: true, method: "POST" do
        div data: stimulus_item(target: :scroll_container, actions: { "scroll" => :check_scroll }, for: Controllers::APP), class: "grow min-h-0 overflow-scroll overscroll-y-contain browser-ios:min-h-auto browser-ios:max-h-none" do
          div class: "px-4 py-4", data: stimulus_item(target: :content_container, for: Controllers::APP) do
            div class: "flex flex-col gap-4" do
              ...
              div data: stimulus_item(target: :tag_results, for: Controllers::ADD), class: "flex flex-col gap-2 empty:hidden"
            end
          end
        end
        ...
        div class: "w-full shrink-0 border-t px-4 py-4 empty:hidden transition group-data-[app-footer-border-value=false]:border-transparent" do
          button data: stimulus_item(target: :submit_button, for: Controllers::ADD), type: "submit", class: "primary-button" do
      ...
      template data: stimulus_item(target: :feed_template, for: Controllers::ADD) do
        div class: "flex" do
          ...
          label class: "flex h-[40px] items-center pr-2" do
            ...
            Checkbox(data: stimulus_item( target: :checkbox, actions: { "change" => :count_selected }, data: { template: "checkbox" }, for: Controllers::ADD ), value: "1")
          end
