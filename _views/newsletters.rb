  ...
  class Newsletters < Jekyll::Component
    ...
    def new_address_form
      div class: "hidden mb-8 group-data-[newsletters-state-value=initial]:block" do
        form data: stimulus_item( target: :form, data: { action: "submit->newsletters#submit:prevent submit->newsletters#disable:prevent " }, for: :newsletters ), action: build_url("create_address"), method: "POST", class: "flex flex-col gap-4", novalidate: true do
          ...
          div do
            label class: "text-input group/address" do
              ...
              input data: stimulus_item( target: :address_input, data: { action: "newsletters#addressInputChanged" }, for: :newsletters ), type: "text", name: "address", autocorrect: "off", autocapitalize: "off", spellcheck: "false", maxlength: "40"
              div data: stimulus_item(target: :numbers, for: :newsletters), class: "px-4 border-l border-400 bg-100 flex items-center justify-center shrink-0 pointer-events-none empty:hidden group-data-[newsletters-address-valid-value=false]:hidden"
            end
          ...
          label class: "text-input" do
            input data: stimulus_item(target: :address_description, for: :newsletters), type: "text", name: "description", placeholder: "Description"
          end
          ...
          button type: "submit", name: "button_action", value: "save", data: stimulus_item(target: :submit_button, for: :newsletters), class: "primary-button mt-4" do
            "Create"
          end
        end
      end
    ...
    def templates
      ...
      template data: stimulus_item(target: :address_template, for: :newsletters) do
        li class: "group flex border-b first:border-t", data: stimulus( controller: :copy, values: { copied: "false", data: "" }, data: { template: "container" } ) do
          button class: "text-left grow min-w-0 flex items-center gap-6 cursor-pointer py-4 outline-2 outline-transparent transition focus-visible:outline-blue-400 focus-visible:rounded", data: stimulus_item( actions: { "click" => :copy }, for: :copy ) do
