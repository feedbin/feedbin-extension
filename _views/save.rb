module Views
  class Save < Jekyll::Component
    ...
    def view_template
      div class: "container group", data: stimulus(controller: :save, actions: { "app:pageInfoError@window" => :load_error, "keydown@document" => :keydown }, values: { state: "initial" }) do
        form class: "container", data: stimulus_item( target: :form, actions: { "submit" => :"submit:prevent" }, for: :save ), action: build_url("save"), method: "POST", novalidate: true do
          # Success message
          ...
          div class: "hidden container group-data-[save-state-value=initial]:flex group-data-[save-state-value=loading]:flex" do
            ...
            div class: "w-full shrink-0 border-t px-4 py-4 empty:hidden transition group-data-[app-footer-border-value=false]:border-transparent" do
              button data: stimulus_item(target: :submit_button, for: :save), type: "submit", class: "primary-button" do
